...
class Servicio
{
  ...
  public function validation($nombre, $descripcion)
  {
      $sql = "CALL ValidateReservesForYear($nombre, '$descripcion');";
      $response=select($sql);
      return $response;
  }
  ...
  public function getServiceAvailableForUser($userid, $serviceid)
  {
    $sql = "SELECT COUNT(sl.id) as 'serviceused' FROM schedule_list sl INNER JOIN servicio_reserva sr ON sl.id = sr.reserva_id WHERE sl.reserved_by = $userid AND sr.servicio_id = $serviceid";
    return ejecutarConsulta($sql);
  }
